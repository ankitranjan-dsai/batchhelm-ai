...
def make_client(
    tmp_path: Path,
    *,
    intake_repository: object | None = None) -> TestClient:
...
def wait_for_intake(
    client: TestClient,
    status_url: str) -> dict[str, object]:
...
def wait_for_run(
    client: TestClient,
    result_url: str) -> dict[str, object]:
    for _attempt in range(100):
...
def test_review_update_and_confirmation_are_versioned(tmp_path: Path) -> None:
    with make_client(tmp_path) as client:
        ...
        updated_response = client.patch(
            f"/api/v1/intakes/{accepted['intake_id']}/draft",
            json={
        ...
        confirmed_response = client.post(
            f"/api/v1/intakes/{accepted['intake_id']}/confirm",
            json={
...
def test_missing_intake_returns_structured_404(tmp_path: Path) -> None:
    with make_client(tmp_path) as client:
        response = client.post(
            "/api/v1/intakes/missing/confirm",
            json={
...
def test_intake_store_failure_does_not_break_health(tmp_path: Path) -> None:
    with make_client(
        tmp_path,
        intake_repository=FailingIntakeRepository()) as client:
    ...
    with make_client(tmp_path) as client:
        ...
        confirmed = client.post(
            ...
            json={
                ...
                "expected_version": review["version"]}).json()
        launch_payload = {
            "request_id": "7d05fc09-d47c-43aa-9f01-b021b26f0ac8"
        }
        first = client.post(
            f"/api/v1/intakes/{accepted['intake_id']}/runs",
            json=launch_payload)
        replay = client.post(
            f"/api/v1/intakes/{accepted['intake_id']}/runs",
            json=launch_payload)
